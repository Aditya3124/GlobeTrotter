...
async function getTopCities() {
  try {
    const res = await fetch('http://localhost:5000/api/cities/popular', { cache: 'no-store' });
    if (!res.ok) return [];
    return res.json();
  } catch (error) {
    console.error(error);
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-24 relative min-h-full">
      ...
      <div className="space-y-6 pt-12">
        <div className="flex justify-between items-end px-2">
          ...
          <Link href="/destinations" className="text-blue-500 font-bold hover:text-blue-600 transition-colors">See all →</Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {topCities.length > 0 ? topCities.map((city: any, i: number) => {
            const imagePool = ["/images/cityscape.jpg", "/images/nature.jpg", "/images/culture.jpg"];
            const imgUrl = imagePool[i % imagePool.length];
            return (
            <div key={city.id} className="bg-white rounded-[2rem] p-4 flex flex-col gap-4 group cursor-pointer shadow-[0_12px_30px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] transition-all duration-300">
              ...
              <div className="px-2 pb-2">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold text-slate-800 group-hover:text-blue-500 transition-colors">{city.city}, {city.country}</h3>
                  <div className="flex items-center gap-1 bg-amber-50 px-3 py-1 rounded-full">
                    <Star className="w-4 h-4 text-amber-500 fill-amber-500" />
                    <span className="text-sm font-bold text-amber-700">4.9</span>
                  </div>
                </div>
                <p className="text-slate-500 text-sm font-medium">Population: {parseInt(city.population).toLocaleString()}</p>
              </div>
            </div>
            );
          }) : (
            <div className="col-span-3 text-center py-10 text-slate-500 font-medium">No top regions found.</div>
          )}
        </div>
      </div>
          ...
          {[1, 2].map((i) => (
            <div key={i} className="bg-white rounded-[2rem] p-5 flex gap-5 group cursor-pointer shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-slate-100 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] transition-all duration-300">
              <div className="w-24 h-24 shrink-0 bg-slate-100 rounded-2xl overflow-hidden relative">
                <img src="/images/paris.jpg" alt="Paris" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
              </div>
